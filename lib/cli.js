...
function main() {
    ...
    optParser.masthead([
        "\0magenta(--- http://sourcemint.org/ ----------------------------------------------",
        "|   \0bold(Sourcemint Open Source Tooling  ~  https://github.com/sourcemint/\0)   |",
...
exports.failAndExit = function(err) {
    if (err && err !== true) {
        if (typeof err === "object" && typeof err.stack !== "undefiend") {
            TERM.stderr.write("\0red(" + err.stack + "\0)\n");
        } else {
            TERM.stderr.write("\0red(" + err + "\0)\n");
        }
    }
    process.exit(1);
}
...
exports.checkPackageProgramPathInArguments = function(args) {
    if (args.length === 0) {
        TERM.stdout.write("\0red(ERROR: No path to package/program specified!\0)\n");
        return;
    }
