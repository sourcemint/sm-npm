                ...
                return gitStatus(pkgInfo[0].path, opts).then(function(gitInfo) {
                    ...
                    if (gitInfo.type !== false) {
                        function formatBranch() {
                            return ((gitInfo.branch !== "master" && (!mappingsUriInfo || !mappingsUriInfo.vendor || gitInfo.branch != mappingsUriInfo.vendor.rev))?("\0orange(" + gitInfo.branch + "\0)"):gitInfo.branch);
                        }
                        function formatTag() {
                            return ((gitInfo.tagged)?" "+gitInfo.tagged:"");
                        }
                        function formatWritable() {
                            return ((gitInfo.writable)?"(\0cyan(W\0))":"");
                        }
                        if (gitInfo.dirty) {
                            ok = false;
                            summary.push("\0red([git" + formatWritable() + ": " + formatBranch() + " \0bold(dirty\0)]\0)");
                        } else
                        if (gitInfo.remoteAhead) {
                            if (context.level === 0) {
                                unsynced = true;
                                summary.push("\0magenta([git" + formatWritable() + ": " + formatBranch() + " \0bold(unpushed\0)]\0)");
                            } else {
                                ok = false;
                                summary.push("\0red([git" + formatWritable() + ": " + formatBranch() + " \0bold(unpushed\0)]\0)");
                            }
                        } else
                        if (gitInfo.branch) {
                            summary.push("\0green([git" + formatWritable() + ": " + formatBranch() + formatTag() + "]\0)");
                        }
                    }
                    ...
                    return npmStatus(pkgInfo[0].path, opts).then(function(npmInfo) {
                        ...
                        if (foundInParent) {
                            // In parent package.
                            TERM.stdout.writenl(padding + summary.join(" ") + "  \0white(<-  " + formatPath() + "\0)" + formatAge() + formatLink());
                        } else
                        ...
                        if (notInstalled) {
                            TERM.stdout.writenl(padding + summary.join(" ") + "  \0red(->  " + formatPath() + "\0)" + formatAge() + formatLink());
                        } else {
                            // Linked package.
                            TERM.stdout.writenl(padding + summary.join(" ") + "  \0cyan(<-  " + formatPath() + "\0)" + formatAge() + formatLink());
                        }
