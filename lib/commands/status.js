...
var SM_STATUS = require("sourcemint-pm-sm/lib/commands/status");
...
command.action(function (options) {
    ...
    SM_PM.forProgramPath(basePath).then(function(pm) {
        ...
        options.time = pm.context.time;

        return SM_STATUS.getStatusTree(pm, options).then(function(statusTree) {
...
if (node.status.summary.isInParent) {
    console.log("FOUND IN PARENT", node.status.summary.isInParent);
}
