...
command.option("--all").bool().help("Show transitive dependencies (dependencies of dependencies).");
command.option("--latest").bool().help("Fetch latest remote info.");
command.option("--dev").bool().help("Include dev dependencies");
command.helpful();
            ...
            return pm.context.program.walkPackages(options, function(parentPkg, pkgInfo, context) {
                ...
                return gitStatus(pkgInfo[0].path, opts).then(function(gitInfo) {
                    ...
                    return npmStatus(pkgInfo[0].path, opts).then(function(npmInfo) {
                        ...
                        if (foundInParent) {
                            // In parent package.
                            TERM.stdout.write(padding + summary.join(" ") + "  \0white(<-  " + formatPath() + "\0)\n");
                        } else
                        if (!parentPkg || pkgInfo[0].path.substring(0, parentPkg.path.length) === parentPkg.path) {
                            ...
                            // Linked package.
                            TERM.stdout.write(padding + summary.join(" ") + "  \0cyan(<-  " + formatPath() + "\0)\n");
                        }
                        ...
                        if (context.level >= 1 && options.all !== true) {
                            return false;
                        }
