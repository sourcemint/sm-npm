...
command.option("--latest").set().help("Check for latest upstream packages.");
...
command.action(function (options) {
    ...
    SM_PM.forProgramPath(basePath).then(function(pm) {
        ...
        var program = pm.context.program;
        var walked = {};
        var hasErrors = false;
        var inDependencies = false;

        return PACKAGES.loadDependenciesForProgram(program).then(function() {
            

program.walkPackages(function(parentPkg, pkgInfo, level, context) {

    console.log("PKG", pkgInfo[2]);
            ...
            function walkPackage(parentPkg, pkgInfo, level, context) {
                ...
                if (pkgInfo[1].indexOf("devDependencies") >= 0) {
                    context.devDependency = true;
                }
                if (context.devDependency === true && options["dev"] !== true && inDependencies) {
                    return;
                }
                inDependencies = true;
                ...
                else if (pkgInfo[1].indexOf("mappings") >= 0 && pkgInfo[1].indexOf("mapped_packages/") === -1) {
                    ok = false;
                    summary.push("\0red([\0bold(defined in 'mappings' but not installed in 'mapped_packages/'\0)]\0)");
                }
                else if (parentPkg) {
                        ...
                        if (!ok) {
                            ...
                            if (options.show !== true) {
                                if (Object.keys(pkgInfo[0].children.packages).length > 0) {
                                    TERM.stdout.write(padding + "    " + "\0magenta(^ --- not going deeper until resolved\0)" + "\n");
                                }
                                return;
                            }
                        }
            ...
            return walkPackage(null, [program.package, [], []], 0, {}).then(function() {
                if (hasErrors) {
