            ...
            return pm.context.program.walkPackages(options, function(parentPkg, pkgInfo, context) {
                ...
                return gitStatus(pkgInfo[0].path, opts).then(function(gitInfo) {
                    ...
                    return npmStatus(pkgInfo[0].path, opts).then(function(npmInfo) {
                        ...
                        if (npmInfo.published) {
                            ...
                            if (npmInfo.usingLatestSatisfying) {
                                ...
                                if (npmInfo.usingLatest) {
                                    ...
                                    summary.push("\0" + redColor + "(1[npm(" + npmInfo.versionSelector + "): \0bold(" + npmInfo.latestSatisfyingVersion + "\0)]\0)");
                                } else if (npmInfo.latestSatisfyingVersion != npmInfo.latestVersion) {
                                    ok = (redColor==="red")?false:ok;
                                    summary.push("\0" + redColor + "(2[npm(" + npmInfo.versionSelector + "): \0bold(" + npmInfo.latestSatisfyingVersion + "\0) \0magenta(new " + npmInfo.latestVersion + "\0)]\0)");
                                } else
                                if (foundInParent) {
                                    unsynced = true;
                        ...
                        function formatLink() {
                            var repositories = pkgInfo[0].descriptor.json.repository;
                            if (!repositories) {
                                repositories = pkgInfo[0].descriptor.json.repositories;
                            } else {
